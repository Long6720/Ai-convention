...
import { ArrowLeft, Upload, type File, Download, Trash2, Eye, Search, FolderOpen, Calendar } from "lucide-react"
...
export default function DocumentManagement() {
  ...
  const [newDocCategory, setNewDocCategory] = useState<DocumentFile["category"]>("standards")
  ...
  const uploadDocument = async () => {
    if (!uploadingFile || !newDocDescription.trim()) {
      toast({
    ...
    try {
      ...
      let content = ""
      if (uploadingFile.type.startsWith("text/") || uploadingFile.type === "application/json") {
        content = await uploadingFile.text()
      }

      const newDocument: DocumentFile = {
        id: Date.now().toString(),
        name: uploadingFile.name.toLowerCase().replace(/\s+/g, "-"),
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">Manage reference documents for code reviews</p>
              </div>
            </div>
        ...
        {showUploadForm && uploadingFile && (
          <Card className="mb-6 border-2 border-blue-200">
            <CardHeader>
              ...
              <CardDescription>Add details for: {uploadingFile.name}</CardDescription>
            </CardHeader>
            ...
            {filteredDocuments.map((doc) => (
              <Card key={doc.id} className="hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-sm truncate" title={doc.originalName}>
                          {doc.originalName}
                        </h3>
                        <div className="flex items-center gap-2 mt-1">
                          <Badge variant="outline" className="text-xs">
                            {documentCategories.find((c) => c.value === doc.category)?.icon}{" "}
                ...
                <CardContent className="pt-0">
                  ...
                  <div className="flex gap-2">
                    {doc.content && (
                      <Button variant="outline" size="sm" onClick={() => previewDoc(doc)} className="flex-1">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <Card className="max-w-4xl max-h-[80vh] w-full">
              ...
              <CardContent className="max-h-[60vh] overflow-auto">
                <pre className="whitespace-pre-wrap text-sm bg-gray-50 p-4 rounded">{previewDocument.content}</pre>
              </CardContent>
            </Card>
          </div>
        )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">
                    {Math.round((documents.reduce((acc, doc) => acc + doc.size, 0) / 1024 / 1024) * 100) / 100}
                  </div>
